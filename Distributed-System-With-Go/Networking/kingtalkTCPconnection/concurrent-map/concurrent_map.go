...
func New[V any](shardCount int) ConcurrentMap[string, V] {
	return create[string, V](shardCount, fnv32)
}

// Creates a new concurrent map.
func NewStringer[K Stringer, V any](shardCount int) ConcurrentMap[K, V] {
	return create[K, V](shardCount, strfnv32[K])
}

// Creates a new concurrent map.
func NewWithCustomShardingFunction[K comparable, V any](shardCount int, sharding func(key K) uint32) ConcurrentMap[K, V] {
	return create[K, V](shardCount, sharding)
}
...
func (m ConcurrentMap[K, V]) Full() bool {
	// count := 0
	// for i := 0; i < m.shardCount; i++ {
	// 	shard := m.shards[i]
	// 	shard.RLock()
	...
	// }
	// return count >= limit
	return int(atomic.LoadInt32(m.count)) >= limit
}
...
// Get retrieves an element from map under given key.
func (m ConcurrentMap[K, V]) MGetWithDefaultValue(keys []K) []V {
	// Get shard
	result := make([]V, 0)
	var defaultV V
...
// Sets the given value under the specified key.
func (m ConcurrentMap[K, V]) SetTTL(key K, value V, ttl time.Duration) {
	// Get map shard.
	shard := m.GetShard(key)
	b := len(shard.items)
	shard.Lock()
